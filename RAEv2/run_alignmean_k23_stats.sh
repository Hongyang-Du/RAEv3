#!/usr/bin/env bash
# Compute latent_stats.pt (Welford mean/var of the TRAINED alignmean-cls-k23-depthattn
# decoder's full-feed encode over ImageNet-256) for the Stage-2 DiT config
# imagenet-dinov3l-alignmean-cls-depthattn-k23.yaml. cls_surrogate:true + loss.align shift
# the latent off N(0,I), so the DiT needs this to standardize (normalization_stat_path in
# the config points here).
#
# Single node, 8 GPU, ~250k samples. Run ONCE before launching job_alignmean_k23_dit_4node.sh.
# Output:
#   /sensei-fs-3/users/hongyangd/ckpt/alignmean-cls-k23-depthattn/latent_stats.pt
#
#   bash run_alignmean_k23_stats.sh
set -euo pipefail
...
NGPU="${NGPU:-8}"
CONFIG=configs/stage2/training/imagenet-dinov3l-alignmean-cls-depthattn-k23.yaml
DATA="${DATA:-$REPO/data/imagenet-256}"
STATS="${STATS:-$ROOT/ckpt/alignmean-cls-k23-depthattn/latent_stats.pt}"
...
echo "========================================================"
echo "  alignmean-cls-k23-depthattn latent stats"
echo "  Config:  ${CONFIG}"
echo "  Data:    ${DATA}"
