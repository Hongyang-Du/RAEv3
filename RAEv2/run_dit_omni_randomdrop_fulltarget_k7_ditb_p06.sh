#!/usr/bin/env bash
# ============================================================
#  DiT-B DECOUPLED FULL TARGET, p_drop=0.6 variant.
#  Same as run_dit_omni_randomdrop_fulltarget_k7_ditb.sh EXCEPT p_drop=0.6
#  (config imagenet-...-p06-ditb.yaml, out dir dit-b-...-p0.6). The decoder is
#  STILL the p0.3 stage-1 decoder (per the p06 config's stage1_ckpt_path); only
#  the combine p_drop and the DiT out_dir differ. transport.decoupled_full_target=true:
#  x_t built from the random-drop latent, FM loss regresses to the full-mean latent.
#  40 epochs, no LR decay. Auto-resumes per EXPERIMENT_NAME from the latest ckpt.
#
#  latent_stats live in the run dir and are already computed; the stats step is
#  skipped when present.
#
#  Usage:
#    bash run_dit_omni_randomdrop_fulltarget_k7_ditb_p06.sh
#    NGPU=8 WANDB_KEY=... bash run_dit_omni_randomdrop_fulltarget_k7_ditb_p06.sh
# ============================================================
set -uo pipefail
...
CFG="configs/stage2/training/imagenet-dinov3l-omni-randomdrop-fulltarget-plain-k7-nano-p06-ditb.yaml"
NAME="dit-b-omni-randomdrop-fulltarget-plain-k7-nano-p0.6"
STATS="$CKPT_ROOT/$NAME/latent_stats.pt"
...
# 1) MARGINAL latent stat under drop=true -- lives in the run dir; skip if present.
if [ ! -f "$STATS" ]; then
...
# 2) train DiT-B. Auto-resumes per EXPERIMENT_NAME. Append (not overwrite) the train.log.
...
EXPERIMENT_NAME="$NAME" ${TR} --nproc_per_node="${NGPU}" --master-port="$(freeport)" \
  ...
  2>&1 | tee -a "$CKPT_ROOT/$NAME/train.log"
