#!/usr/bin/env bash
# Compute latent_stats.pt (Welford mean/var of the FROZEN bandjepa-k23 combine's
# full-feed encode over ImageNet-256) for the Stage-2 DiT config
# imagenet-dinov3l-bandjepa-depthattn-k23.yaml. cls_surrogate:true adds the deep-layer
# token-mean to every position, shifting the latent off N(0,I), so the DiT needs this to
# standardize (normalization_stat_path in the config points here).
#
# Single node, 8 GPU, ~250k samples (matches the fusion-jepa-k23 recipe). Run ONCE before
# launching job_bandjepa_k23_dit_4node.sh. Output:
#   /sensei-fs-3/users/hongyangd/ckpt/stage0-bandjepa-depthattn-k23/latent_stats.pt
#
#   bash run_bandjepa_k23_stats.sh
set -euo pipefail
...
NGPU="${NGPU:-8}"
CONFIG=configs/stage2/training/imagenet-dinov3l-bandjepa-depthattn-k23.yaml
# on-the-fly configs read the relative ./data/imagenet-256 (symlink -> node-local staged copy)
DATA="${DATA:-$REPO/data/imagenet-256}"
STATS="${STATS:-$ROOT/ckpt/stage0-bandjepa-depthattn-k23/latent_stats.pt}"
...
echo "========================================================"
echo "  bandjepa-k23 latent stats"
echo "  Config:  ${CONFIG}"
echo "  Data:    ${DATA}"
