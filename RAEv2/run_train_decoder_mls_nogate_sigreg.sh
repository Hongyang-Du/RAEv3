#!/usr/bin/env bash
# ============================================================
#  raev2 FIXED MLS + Projector(SIGReg) + Decoder training  (NO GATE)
#  (src/train_decoder_mls_nogate_sigreg.py)
#
#  Encoder:  DINOv3-L (frozen, layers 11,13,15,17,19,21,23)
#  Combine:  fixed unweighted MEAN over K layers (no learnable gate)
...
SCRIPT=$(dirname "$(realpath "$0")")/src/train_decoder_mls_nogate_sigreg.py
...
OUT_DIR=output_full/train_decoder_mls_nogate_sigreg
...
echo "========================================================"
echo "  FIXED MLS (no gate) + Projector(SIGReg) + Decoder training"
