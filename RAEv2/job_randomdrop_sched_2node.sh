#!/usr/bin/env bash
# 2-node (2 x 8 GPU) stage-1 decoder training with a SCHEDULED random-drop rate.
# Everything else is identical to the nano random-drop k23 recipe; only p_drop is annealed
# start -> end over training (see configs/.../random-drop-layer-mls-plain-k23-nano-sched.yaml).
#
# Pluto job: replicas = 2, GPUs/replica = 8. Paste this whole block into the job "Scripts" field.
# Flip direction or change endpoints WITHOUT editing the config via the PDROP_* env vars below
# (e.g. 0.9 -> 0.1, or cosine): they override the YAML schedule.
set -exo pipefail
...
# ---- random-drop schedule (a -> b). Edit these two lines to pick any range/direction. ----
export PDROP_START=0.1        # p_drop at the start of training
export PDROP_END=0.9          # p_drop at the end of training  (set 0.9 / 0.1 to reverse)
...
export OUT_DIR_OVERRIDE=/sensei-fs-3/users/hongyangd/ckpt/omni-randomdrop-plain-k23-nano-sched-${PDROP_START}to${PDROP_END}-2node
