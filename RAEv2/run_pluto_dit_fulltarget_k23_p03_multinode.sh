...
#  k23 plain, p_drop=0.3.  Per-node entry: EVERY Pluto replica runs this exact
...
#    CFG (default): configs/stage2/training/imagenet-dinov3l-omni-randomdrop-fulltarget-plain-k23-p03-ditb.yaml
...
#          -> dit-b-omni-randomdrop-fulltarget-plain-k23-p0.3
...
#    bash /sensei-fs-3/users/hongyangd/RAEv3_oldnorm/RAEv2/run_pluto_dit_fulltarget_k23_p03_multinode.sh
# ============================================================
set -uo pipefail
...
# is readable from anywhere. One file per pod: logs/<job>-k23-p03-node<RANK>.log
mkdir -p "$ROOT/logs"
exec > >(tee -a "$ROOT/logs/${JOB_NAME:-dit-fulltarget-k23-p03}-node${RANK:-0}.log") 2>&1
...
CFG="${CFG:-configs/stage2/training/imagenet-dinov3l-omni-randomdrop-fulltarget-plain-k23-p03-ditb.yaml}"
...
exec "$TR" \
  ...
  --max-restarts=3 \
  --rdzv_backend=c10d \
  --rdzv_id="${JOB_NAME:-dit-fulltarget-k23-p03}-${NAME}" \
