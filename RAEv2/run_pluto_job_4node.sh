...
case "${1:-}" in
  ...
  exp6) CFG=configs/stage2/training/imagenet-dinov3l-h1decoder-p095-k23.yaml;         BASE=omnirae-dit-h1-p095-k23 ;;  # DDT-XL DiT on h1 (first decoder block) of the p=0.95 drop-rate omni-randomdrop-plain k23 decoder. Same recipe as imagenet-dinov3l-h1decoder-k23.yaml. Precompute h1 stats once first: torchrun --nproc_per_node=1 src/compute_h1_stats.py --config configs/stage2/training/imagenet-dinov3l-h1decoder-p095-k23.yaml --num-samples 50000 --out /sensei-fs-3/users/hongyangd/ckpt/h1-p095-k23-block0/h1_stats.pt
  ...
  jepa-k7-cls) CFG=configs/stage2/training/imagenet-dinov3l-jepa-depthattn-k7-cls.yaml; BASE=omnirae-dit-jepa-depthattn-k7-cls ;;  # DiT on the CLS-ON JEPA k7 latent (cls_surrogate:true) that ckpt/stage1-decoder-jepa-depthattn-k7-cls inverts; viz ON (ema_dec). NEEDS latent_stats.pt (run run_jepa_k7_cls_stats.sh once)
  ...
  *) echo "usage: NUM_NODES=4 bash run_pluto_job_4node.sh <exp1|exp2|exp3|exp4|exp5|exp6|jepa-k7|jepa-k23|bandjepa-k23|rent-ganfusion|alignmean-k23>"; exit 1 ;;
esac
