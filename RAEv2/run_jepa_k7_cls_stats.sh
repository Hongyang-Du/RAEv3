#!/usr/bin/env bash
# Compute latent_stats.pt (Welford mean/var of the FROZEN JEPA-k7 combine + cls_surrogate
# full-feed encode over ImageNet-256) for the Stage-2 DiT config
# imagenet-dinov3l-jepa-depthattn-k7-cls.yaml. cls_surrogate:true adds the L23 token-mean
# to every position, shifting the latent off N(0,I), so the DiT needs this to standardize
# (normalization_stat_path in the config points here).
#
# Single node, 8 GPU, ~250k samples. Run ONCE before launching the DiT job. Output:
#   /sensei-fs-3/users/hongyangd/ckpt/stage1-decoder-jepa-depthattn-k7-cls/latent_stats.pt
#
#   bash run_jepa_k7_cls_stats.sh
set -euo pipefail
...
NGPU="${NGPU:-8}"
CONFIG=configs/stage2/training/imagenet-dinov3l-jepa-depthattn-k7-cls.yaml
DATA="${DATA:-$REPO/data/imagenet-256}"
STATS="${STATS:-$ROOT/ckpt/stage1-decoder-jepa-depthattn-k7-cls/latent_stats.pt}"
