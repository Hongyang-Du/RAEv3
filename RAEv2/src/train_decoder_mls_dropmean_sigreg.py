...
def main():
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if is_main and val_layers_fixed is not None:
            ...
            # layer-usage probe (the gate-free collapse readout): the combine has no
            # params, so the same nets decode any layer subset. LOO dPSNR_i = how much
            # the decoder relies on layer i; solo PSNR_i = layer i alone. Collapse
            # signature: one layer dominates both (huge LOO drop, others ~0).
            with torch.no_grad():
