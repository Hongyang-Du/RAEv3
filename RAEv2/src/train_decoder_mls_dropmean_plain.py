...
def main():
    ...
    parser.add_argument("--val-npz",      type=str,   default="data_eval/imagenet-256-val.npz",
                        help="ImageNet val npz; per-epoch val PSNR+SSIM on a random subset")
    parser.add_argument("--val-n",        type=int,   default=1000,
                        help="Number of random val images for the per-epoch val PSNR/SSIM")
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if is_main and val_layers_fixed is not None:
            ...
            print(f"  Val PSNR (EMA, {val_rec.shape[0]} demo imgs): {val_ps:.2f} dB", flush=True)
            if args.wandb:
                import wandb
                wandb.log({"val/psnr_demo": val_ps}, step=global_step)

        # -- val-set PSNR (+SSIM): random N val images, paper protocol ----------
        if is_main and args.val_npz and os.path.exists(args.val_npz):
            def _recon(imgs01):
                toks = encode_layers(imgs01)
                z = ema_proj(toks)                          # eval = full mean
