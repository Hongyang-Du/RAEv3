#!/usr/bin/env python3
"""
Multi-GPU DDP training: raev2 MLS latent -> SOFTMAX-gated Projector (SIGReg) -> ViT Decoder.

Anti-collapse gated variant of train_decoder_mls_sigreg.py: the K MLS layers are
combined by a SOFTMAX convex weighting + per-step LAYER DROPOUT (--layer-drop), so the
per-layer gate cannot collapse to the shallowest layer (which a free sigmoid gate does).
A small learnable projector then maps the combined latent to the SIGReg space.
...
Usage:
    torchrun --nproc_per_node=8 src/train_decoder_mls_softgate_sigreg.py \
        --data /datasets/imagenet-256 \
        --epochs 100 --batch-size 64 --sigreg-w 1 --layer-drop 0.2 --wandb
"""
...
class MLSProjector(nn.Module):
    """Softmax per-layer gate (+ layer dropout) over the K MLS layers + per-token residual MLP.

    Anti-collapse gated variant: layers are combined by a SOFTMAX convex weighting
    (weights sum to 1, init uniform 1/K) and, during training, a random subset of layers
    is dropped each step (layer dropout, prob p_drop) so the model can't lean on a single
    layer — keeping the weights spread instead of collapsing to the shallowest layer
    (which a free sigmoid gate does). SIGReg shapes the projector output toward N(0, I).
    """
    def __init__(self, n_layers: int, dim: int = 1024, out_dim: int = 1024,
                 mult: int = 4, p_drop: float = 0.2):
        super().__init__()
        self.p_drop = p_drop
        self.gate = nn.Parameter(torch.zeros(n_layers))     # softmax logits, init 0 -> uniform 1/K
    ...
    def forward(self, layer_tokens) -> torch.Tensor:        # K x [B, N, dim] -> [B, N, out_dim]
        ...
        w = torch.softmax(self.gate, dim=0)                 # [K]  convex weights (sum=1)
        if self.training and self.p_drop > 0:               # layer dropout: forces spread
            keep = torch.rand(stk.shape[0], device=stk.device) > self.p_drop
            if not keep.any():                              # always keep >= 1 layer
                keep[torch.randint(stk.shape[0], (1,), device=stk.device)] = True
            w = w * keep
            w = w / w.sum().clamp_min(1e-6)                 # renormalize over kept layers
        z0 = (w.view(-1, 1, 1, 1) * stk).sum(0)            # [B, N, dim]  softmax-weighted mix
...
def main():
    ...
    parser.add_argument("--layer-drop",   type=float, default=0.2,
                        help="Per-step layer-dropout prob (anti-collapse; 0 disables)")
    parser.add_argument("--log-every",    type=int, default=50)
    parser.add_argument("--val-every",    type=int, default=500)
    parser.add_argument("--ckpt-every",   type=int, default=2)
    parser.add_argument("--out-dir",      default="output/train_decoder_mls_softgate_sigreg")
    ...
    if args.wandb and is_main:
        ...
        wandb.init(
            ...
            name=f"decoder-mls-softgate-sigreg-k{len(args.layers)}",
            config={**vars(args), "global_batch": args.batch_size * world_size},
            tags=["decoder", "mls", "sigreg", "softgate", "layerdrop", "raev2", "stage1", f"k{len(args.layers)}"],
        )
    ...
    def encode_layers(imgs01: torch.Tensor):
        ...
        These feed the learnable gate in MLSProjector (replacing the encoder's frozen
    ...
    projector = MLSProjector(n_layers=len(args.layers), p_drop=args.layer_drop,
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for step, (imgs, _) in enumerate(train_loader):
            ...
            # -- Step A: Generator (gated projector + decoder) -----------------
            optimizer.zero_grad(set_to_none=True)

            with autocast_ctx:
                z = projector_ddp(layer_tokens)               # gate+combine+project (trainable)
            ...
            if is_main and global_step % args.log_every == 0:
                ...
                zd = gaussian_diag(z)
                ps = psnr(x_rec, imgs)
                gate_sig = torch.softmax(projector.gate, dim=0).detach().tolist()  # per-layer softmax weight
