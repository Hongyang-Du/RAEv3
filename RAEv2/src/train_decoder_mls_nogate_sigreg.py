#!/usr/bin/env python3
"""
Multi-GPU DDP training: raev2 FIXED MLS latent -> learnable Projector (SIGReg) -> ViT Decoder.

No-gate variant of train_decoder_mls_sigreg.py: the K MLS layers are combined by a
FIXED unweighted mean (RAEv2 style), NOT a learnable per-layer gate (a free gate
collapses to the shallowest layer under reconstruction). A small learnable projector
sits between the fixed MLS latent and the decoder and is regularized with SIGReg.
...
class MLSProjector(nn.Module):
    """Fixed MLS combine (RAEv2: unweighted mean over K layers) + per-token residual MLP.

    NO learnable gate (that's the gated variant, train_decoder_mls_sigreg.py). A free
    per-layer gate collapses to the shallowest layer under a reconstruction objective,
    defeating multi-layer fusion; the fixed mean keeps all K layers in by construction
    (matches RAEv2). This residual MLP is the only learnable module between the frozen
    encoder and the decoder, so it is what SIGReg shapes toward N(0, I).
    """
    ...
    def forward(self, layer_tokens) -> torch.Tensor:        # K x [B, N, dim] -> [B, N, out_dim]
        z0 = torch.stack(layer_tokens, dim=0).mean(0)       # [B, N, dim]  fixed MLS mean (no gate)
...
def main():
    ...
    parser.add_argument("--out-dir",      default="output/train_decoder_mls_nogate_sigreg")
    ...
    if args.wandb and is_main:
        ...
        wandb.init(
            ...
            name=f"decoder-mls-nogate-sigreg-k{len(args.layers)}",
            config={**vars(args), "global_batch": args.batch_size * world_size},
            tags=["decoder", "mls", "sigreg", "nogate", "raev2", "stage1", f"k{len(args.layers)}"],
        )
    ...
    def encode_layers(imgs01: torch.Tensor):
        ...
        These feed MLSProjector, which combines them by a fixed unweighted mean.
        Same per-layer extraction the MLS encoder did internally.
        """
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for step, (imgs, _) in enumerate(train_loader):
            ...
            with autocast_ctx:
                z = projector_ddp(layer_tokens)               # fixed MLS mean + project (trainable)
