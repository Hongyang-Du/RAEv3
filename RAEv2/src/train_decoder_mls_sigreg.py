#!/usr/bin/env python3
"""
Multi-GPU DDP training: raev2 MLS latent -> learnable Projector (SIGReg) -> ViT Decoder.

Same recipe as train_decoder_mls.py, but inserts a small learnable projector
between the (fixed) MLS latent and the decoder, and regularizes the projector
output with SIGReg. SIGReg only makes sense here because the projector is the
trainable module it shapes (on a fixed MLS it would have zero gradient).
...
Usage:
    torchrun --nproc_per_node=8 src/train_decoder_mls_sigreg.py \
        --data /datasets/imagenet-256 \
        --epochs 100 --batch-size 64 --sigreg-w 0.1 --wandb
"""
...
class MLSProjector(nn.Module):
    """Learnable per-layer gate over the K MLS layers (init half-open) + per-token residual MLP.
    ...
    each layer reaches the decoder. The gated combine then goes through a residual MLP.
    This is the only learnable module between the frozen encoder and the decoder, so it
    is what SIGReg shapes toward N(0, I).
    """
...
def main():
    ...
    parser.add_argument("--out-dir",      default="output/train_decoder_mls_sigreg")
    ...
    if args.wandb and is_main:
        ...
        wandb.init(
            ...
            name=f"decoder-mls-sigreg-k{len(args.layers)}",
            config={**vars(args), "global_batch": args.batch_size * world_size},
            tags=["decoder", "mls", "sigreg", "raev2", "stage1", f"k{len(args.layers)}"],
        )
