#!/usr/bin/env bash
# ============================================================
#  Unified stage-1 decoder training launcher.
#  Usage:  bash run_decoder.sh <config.yaml> [NGPU]
#  GPUs:   set CUDA_VISIBLE_DEVICES to pick cards (default 0-3).
#
#  e.g.  CUDA_VISIBLE_DEVICES=4,5,6,7 bash run_decoder.sh \
#            configs/stage1/decoder/dropmean-bn-sigreg-k23.yaml
# ============================================================
set -euo pipefail

CONFIG=${1:?usage: run_decoder.sh config.yaml [NGPU]}
...
echo "========================================================"
echo "  decoder training  config=${CONFIG}"
...
PYTORCH_ALLOC_CONF=expandable_segments:True ${TORCHRUN} --nproc_per_node=${NGPU} \
    ...
    src/train_decoder.py --config "${CONFIG}"
