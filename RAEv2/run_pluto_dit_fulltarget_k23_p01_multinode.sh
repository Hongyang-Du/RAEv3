...
#  k23 plain, p_drop=0.1.  Per-node entry: EVERY Pluto replica runs this exact
...
#    CFG (default): configs/stage2/training/imagenet-dinov3l-omni-randomdrop-fulltarget-plain-k23-p01-ditb.yaml
#      * global_batch_size=2048 is the invariant. train.py derives
#        micro = 2048/(world_size*grad_accum_steps). Constraint: 2048 % (NUM_NODES*NUM_OF_GPUS) == 0.
#      * decoder = config stage1_ckpt_path (shared k23-nano p0.3 stage-1 decoder).
#      * latent_stats.pt = config normalization_stat_path.
#        UNLIKE p0.3, the p0.1 stats are NOT precomputed. On a SINGLE-NODE launch this
#        script computes them once (torchrun --standalone) before training. On a
#        MULTI-NODE launch it FATALs and asks you to precompute first (stats compute is
#        a single-node standalone job; don't run it under the DDP rendezvous group).
#      * EXPERIMENT_NAME = basename(dirname(normalization_stat_path))
#          -> dit-b-omni-randomdrop-fulltarget-plain-k23-p0.1
...
# is readable from anywhere. One file per pod: logs/<job>-k23-p01-node<RANK>.log
mkdir -p "$ROOT/logs"
exec > >(tee -a "$ROOT/logs/${JOB_NAME:-dit-fulltarget-k23-p01}-node${RANK:-0}.log") 2>&1
...
CFG="${CFG:-configs/stage2/training/imagenet-dinov3l-omni-randomdrop-fulltarget-plain-k23-p01-ditb.yaml}"
...
# latent_stats.pt: p0.1 is NOT precomputed. Compute once on a single-node launch;
# refuse to guess under a multi-node rendezvous group (run it standalone first instead).
if [ ! -f "$STATS" ]; then
  if [ "${NUM_NODES}" -ne 1 ]; then
    echo "### FATAL: latent_stats missing: $STATS"
    echo "###        Multi-node launch won't compute it (stats is a single-node standalone job)."
    echo "###        Precompute once first, e.g.:"
    echo "###          $TR --standalone --nproc_per_node=8 scripts/stage1/compute_latent_stats.py \\"
    echo "###            --config $CFG --data-dir $LSSD --output-path $STATS --num-samples 250000"
    exit 1
  fi
  echo "### $(date '+%F %T') computing latent_stats (single node) -> $STATS"
  mkdir -p "$(dirname "$STATS")"
  "$TR" --standalone --nproc_per_node="$NPROC" scripts/stage1/compute_latent_stats.py \
    --config "$CFG" --data-dir "$LSSD" --output-path "$STATS" --num-samples 250000 \
    || { echo "### FATAL: compute_latent_stats failed"; exit 1; }
else
  echo "### latent_stats already present: $STATS"
fi
[ -f "$STATS" ] || { echo "### FATAL: stats missing after compute: $STATS"; exit 1; }
...
exec "$TR" \
  ...
  --max-restarts=3 \
  --rdzv_backend=c10d \
  --rdzv_id="${JOB_NAME:-dit-fulltarget-k23-p01}-${NAME}" \
